...
### v1.3.1 (最新版)
- 💼 **职位描述扩展**：更新了所有飞书日程的描述、会议标题及前端 UI 文案，以支持 `Product Manager (产品经理)` 和 `Intern (实习生)` 双重岗位的招聘需求，扩大了系统的适用范围。

### v1.3.0
- 📄 **简历附件上传支持**：在前端预约表单中新增了简历 PDF 文件上传入口（最大支持 5MB）。
- 📎 **飞书日历附件同步**：候选人上传的简历将被系统自动上传至飞书云文档，并直接绑定到最终生成的飞书面试日程中，方便面试官在会前随时查阅。

### v1.2.1
- ⏰ **时区修正**：在生成日历时间戳时显式声明了 `Asia/Shanghai`（北京时间）。这修复了当项目部署在默认使用 UTC 时区的云平台（如 Vercel/Railway）上时，晚间的面试（如 21:00）会被错误地推迟 8 小时变成次日凌晨（05:00）的严重 Bug。

### v1.2.0
- 💼 **职位名称更新**：将所有飞书日程的描述及发送给候选人的邮件主题更新为具体的招聘岗位：`TCL FALCON Overseas Product Operation Intern`。
- 📅 **飞书日历深度整合**：
  - 成功开启并接入应用机器人 (Bot) 能力，彻底跑通飞书主日历事件写入流程。
  - 📧 **全自动邮件邀请**：现已将候选人填写的邮箱作为外部第三方参与人 (`third_party`) 加入日程。系统将自动触发一封正式的面试邀请邮件至候选人邮箱，并附带可一键添加到本地日历的 `.ics` 文件。
  - 👀 **公开参会人列表**：配置了 `can_see_others` 属性，取消了“隐藏参与者”限制，让面试更具正式感与透明度。

### v1.1.1
- 🐛 **修复 Bug**：修复了当候选人输入中文姓名时，由于 `requests` 默认使用 `latin-1` 编码导致请求飞书 API 报错崩溃的问题。现已全面强制采用 UTF-8 编码处理数据。
